...
if (locality.lvl == "ZCTA"){
  ...
  our_metro_df <- our_metro_df %>% select(created_at, beach_id, beach_name, location, deadfish_num, deadfish_num)
  ...
  timeline_overall <- timeline_overall %>%
    ...
    complete(ZCTA, Date = date.seq, fill=list(deadfish_num=0)) %>%
    group_by(ZCTA, Date) %>% 
  ...
  ##  First, initializing it with beach condition data, 
...
if (locality.lvl == "City"){
  ...
  for (ZCTA.code in left.out.TB.area.zctas){
    ...
    interm.df %>% complete(created_at = seq.Date(min(affected_ZCTA_df$created_at), max(affected_ZCTA_df$created_at), by="day"))
    interm.df[,c("beach_id", "beach_name", "location")] <- "None"
    interm.df[,c("deadfish_num", "deadfish_num")] <- 0
    interm.df$ZCTA <- ZCTA.code
    unaffected_ZCTA_df <- rbind(unaffected_ZCTA_df, interm.df)
  }
  ...
  timeline_overall <- timeline_overall %>%
    ...
    complete(primary_city, Date = date.seq, fill=list(deadfish_num=0)) %>%
  ...
  model.df <- timeline_overall %>%
    select(primary_city, Date, mean.deadfish_num)
...
if (locality.lvl == "Total"){
  ...
  timeline_overall <- timeline_overall%>%
    complete(Date = date.seq, fill=list(deadfish_num=0)) %>%
    group_by(Date) %>% 
    summarise(median.deadfish_num = median(deadfish_num),
              max.deadfish_num = max(deadfish_num),
              mean.deadfish_num = mean(deadfish_num),
  ...
  ## POST-FACTUM correlations (correlation of tweets with PREVIOUS TIME PERIOD's Deadfish Levels data)
  postfactum.corr <- model.df %>%
    mutate(mean.deadfish_num.lag = lag(mean.deadfish_num, n=1)) %>%
    select(-Date) %>%
    cor(use="complete.obs") %>% .[,"mean.deadfish_num.lag"] %>% round(2) %>% .[c(2,3)]
